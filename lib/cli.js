...
program
	...
	.action(async (projectName, cmdObj) => {
		...
		let docker = await select({
			message: 'Would you like to include a Dockerfile?',
			choices: ['Yes', 'No'],
		});

		let eslint = await select({
			message: 'Would you like to include ESLint configuration?',
			choices: ['Yes', 'No'],
		});

		let entrypoint = await select({
			message: 'Would you like to include Entrypoint?',
			choices: ['Yes', 'No'],
		});

		docker = docker === "Yes" ? true : false;
		eslint = eslint === "Yes" ? true : false;
		entrypoint = entrypoint === "Yes" ? true : false;
		...
		await createDirectoryStructure(projectPath, { projectName, description, version, docker, eslint, entrypoint });
	});
