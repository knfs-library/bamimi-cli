...
async function generateDocker() {
	try {
		...
		console.log(`Docker created successfully at ${path.join(process.cwd(), 'Dockerfile')}`);
	} catch (error) {
		console.error(error)
	}
}
...
async function generateTestConfig() {
	try {
		...
		console.log(`TestConfig created successfully at ${path.join(process.cwd(), 'jest.config.js') }`);
	} catch (error) {
		console.error(error)
	}
}
...
async function generateEslint() {
	try {
		...
		fs.writeJsonSync(packageJsonPath, packageJson, { spaces: 2 });
		console.log(`Eslint Config created successfully at ${path.join(process.cwd(), 'eslint.config.js')}`);
	} catch (error) {
		console.error(error)
	}
}
...
async function generateSQLConfig() {
	try {
		...
		console.log(`SQL Config created successfully at ${path.join(process.cwd(), '.sequelizerc')}`);
	} catch (error) {
		console.error(error)
	}
}
