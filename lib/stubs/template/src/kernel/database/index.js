 ...
 */
const get = (options = { useDatabase: null }) => {
	/**
	...
	const useDBs = {
		...
		nosql: function () {
			switch (config.nosql.connection) {
				case "mongodb":
					return require("./mongodb")(config.nosql.mongodb)
			}
		}
	}

	const db = options.useDatabase ? useDBs[options.useDatabase]() : useDBs[config.useDatabase]();
